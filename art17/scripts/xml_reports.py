...
def get_current_checklist_id():
    cnf = models.Config.query.filter_by(id='REPORTING_ID').first()
    return int(cnf.value) if cnf and cnf.value else None
...
def xml_species_checklist(filename=None):
    checklist_id = get_current_checklist_id()
    species = models.db.session.query(models.DataSpeciesCheckList).filter_by(
        member_state='RO',
...
def xml_habitats_checklist(filename=None):
    checklist_id = get_current_checklist_id()
    habitats = models.db.session.query(models.DataHabitatsCheckList).filter_by(
        member_state='RO',
        dataset_id=checklist_id,
    )
    habitats_as_dict = {}
